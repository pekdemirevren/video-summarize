  ...
  return (
    <button
      className={`relative flex items-center justify-center w-full px-4 py-3 font-semibold text-white rounded-lg shadow-md transition-all duration-300 ease-in-out
        ${props.disabled ? 'bg-base-300/70 text-content/50 cursor-not-allowed' : 'bg-brand-primary hover:bg-brand-primary/80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-base-100 focus:ring-brand-primary'}
        ${className}`}
      {...props}
    >
      {isLoading ? (
